...
5. **Steering reminder.** If the agent leaves marked results un-replaced, toolclip observes the pile at each `turn_end` (after the turn's tool results are in — both count and total estimated tokens) and, when either crosses its threshold (count > 5, or total > 5000 estimated tokens; each once per excursion via independent latches), delivers a reminder through **pi's native steering** — `sendUserMessage(..., {deliverAs: "steer"})`. Pi persists the steer as a real `user` message at the next turn boundary: it becomes part of the session's messages, is visible in every subsequent LLM call, and is included in compaction/summarization — until the pile is distilled. The reminder summarizes the pile — "you have N tool-result-pending-replacements totalling ~T estimated tokens" — and lists the pending ids with their sizes so the agent can act in one batched call. When the pile falls back to (or below) a threshold, that trigger's latch re-arms, so a re-grown pile is nagged again. Latches reset at each round boundary, so a pile persisting into a new round is re-announced at that round's first turn boundary. (A pile above threshold at a run's final turn forces one more turn — the model must at least see the nag.)
...
| `TOOLCLIP_STEERING_REMINDER` | `true` | Deliver a steering reminder via pi's native steering when either trigger fires (count or size), listing the pending ids with their sizes. |
