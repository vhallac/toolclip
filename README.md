...
2. The LLM can call `replace_tool_result(toolCallId, replacement)` at its leisure. **No length gate is applied** — replacements of any size are accepted. The tool records a `grew` flag (`replacementTokens >= originalTokens`) in its details so observers can detect replacements that bloat rather than shrink context. That flag is the signal for reintroducing a length gate later.
   ...
   An **empty (or whitespace-only) replacement** is the model's "this result was useless" signal: the placeholder text (`TOOLCLIP_EMPTY_REPLACEMENT_TEXT`, default `tool result was not useful`) is stored in place, and the original is swapped for that text plus the replaced marker — never a pointer, and no receipt is minted for it (nothing was stored in the replace call's arguments; re-replacing a previously stamped entry clears its stamps so it falls back to the in-place form). The store still counts: the original leaves the steering pile. A placeholder deliberately does not feed the expiry `R` (the running mean replacement size).
3. On the `context` event (before each LLM call), any `ToolResultMessage` whose `toolCallId` has a stored replacement is swapped. In **pointer mode** (default, `TOOLCLIP_REPLACEMENT_MODE`) the replacement text is kept exactly ONCE in context — in the model's own `replace_tool_result` call arguments, which are never modified — and the original becomes a pointer to it:
   ```
   [tool-result-replaced: toolCallId=abc; summary is the replacement text for this id in your replace_tool_result call call_A, receipt rp7k2-1]
   ```
   The replace call's echo carries the receipt — `Receipt rp7k2-1: stored 2 replacement(s) (call call_A). Originals are swapped for a pointer to this call's arguments in subsequent LLM calls:` — and every item it stored is stamped with the call's id and receipt, so the model resolves the pointer by finding its own call whose result carries the same receipt (the receipt is the reliable key: some chat templates never show call ids). A re-replacement re-points the entry at the newest call. If the pointer's target call is no longer in the messages (compaction, a fork, another extension), the swap **falls back** to the copy form below — a pointer must never dangle. In **copy mode** (`TOOLCLIP_REPLACEMENT_MODE=copy`, for A/B runs) — and in the fallback — the swapped result is:
   ```
   <replacement>
   [tool-result-replaced: toolCallId=abc]
   ```
   The replaced-marker prefix (`[tool-result-replaced: toolCallId=...`) is kept identical in both forms for traceability. Cache breaks here — by design.
...
| `TOOLCLIP_EXPIRY_REPLACEMENT_COPIES` | `1` (pointer) / `2` (copy) | Copies of the replacement text in context for the expiry pay-back formula: 1 in pointer mode (the text lives once, in the replace call's args), 2 in copy mode (args + swapped result). An explicit env value always wins. |
| `TOOLCLIP_EXPIRY_OVERHEAD_TOKENS` | `95` (pointer) / `60` (copy) | Fixed tokens per replacement not otherwise counted (call block, ids, echo; pointer mode adds ~35 for the pointer replacing the old two-block marker). An explicit env value always wins. |
...
| `TOOLCLIP_REPLACEMENT_MODE` | `pointer` | How a replaced original is presented: `pointer` — the replacement text is kept once (in the replace call's args) and the original becomes a pointer naming that call and its receipt, with the copy-form fallback when the target call left the messages; `copy` — the summary text is written in both places (the pre-pointer behavior, for A/B runs). |
| `TOOLCLIP_POINTER_INCLUDE_CALL_ID` | `true` | Include the replace call's toolCallId in the pointer text. Some chat templates never show call ids to the model; the receipt is always included and is the reliable key. |
| `TOOLCLIP_RECEIPT_PREFIX` | `"rp"` | Prefix of the receipt id (`rp7k2-3` → prefix `rp`). |
| `TOOLCLIP_RECEIPT_TAG_LENGTH` | `3` | Random base36 characters in the receipt id, generated once per extension load so receipts stay unique after a resume (the counter restarts). `0` disables the tag. |
| `TOOLCLIP_EMPTY_REPLACEMENT_TEXT` | `"tool result was not useful"` | Text stored in place of an empty or whitespace-only replacement — the model's "this result was useless" signal. The original is swapped in place for this text plus the replaced marker (no pointer, no receipt stamp); the store still resets the steering pile, and the placeholder never feeds the expiry R. |
