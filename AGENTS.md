...
- Registers a `replace_tool_result(toolCallId, replacement)` tool the LLM can call at its leisure. **No length gate is applied** — replacements of any size are accepted, and the tool records a `grew` flag (`replacementTokens >= originalTokens`) in its details as the observation target for reintroducing a gate later. An **empty (or whitespace-only) replacement** means "the result was useless": the placeholder text (`TOOLCLIP_EMPTY_REPLACEMENT_TEXT`, default `tool result was not useful`) is stored in place, the entry is never stamped (the context swap uses the in-place form — re-replacing a stamped entry clears its stamps), no receipt is minted for a placeholder-only call, and `observeReplacement` is skipped (a placeholder never pulls the expiry R down); the store still resets the steering pile. A call that stores at least one replacement mints a **receipt** (`prefix + random base36 tag + counter`, e.g. `rp7k2-3`; lib/receipt.ts): the echo header becomes `Receipt rp7k2-1: stored N replacement(s) (call <callId>). ...` in pointer mode (copy mode keeps the old header), and the details carry `{ mode, receiptId, replaceCallId }` (receipt fields only when something was stamped). Each stamped entry carries the storing call's `replaceCallId` and `receiptId`; a re-replacement overwrites both. A call that stamps nothing (nothing stored, or only placeholders) mints no receipt and leaves the counter untouched.
...
| `TOOLCLIP_EMPTY_REPLACEMENT_TEXT` | `"tool result was not useful"` | Text stored in place of an empty or whitespace-only replacement (the "useless result" signal): swapped in place with the replaced marker, no pointer/receipt, never feeds the expiry R; the store still resets the steering pile. |
...
- An empty (whitespace-only) replacement is the model's "useless result" signal, not a malformed call: the placeholder text (`TOOLCLIP_EMPTY_REPLACEMENT_TEXT`) is stored in place (no pointer, no receipt stamp — nothing lives in the replace call's args), a placeholder never feeds the expiry R (`replMeanTokens`), and the store still resets the steering pile. `pendingIds` must test `replacement === undefined`, not falsiness — a configured empty-string placeholder is a replacement, not a pending.
